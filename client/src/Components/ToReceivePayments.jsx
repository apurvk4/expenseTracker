...
const ToReceivePayments = () => {
  ...
  async function getData() {
    setLoading(true);
    const url = new URL(
      import.meta.env.VITE_DEPLOYMENT + "/api/user/toreceivemoneylist"
    );
  ...
  function toReceiveMoney(row) {
    let l = row.friends.length;
    let perPerson = row.amount / l;
    let value = row.amount;
    for (let i = 0; i < row.friends.length; i++) {
      if (row.friends[i].paid) {
        value -= perPerson;
      }
    }
